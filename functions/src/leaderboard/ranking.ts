...
import { formatWeekShort } from "./weekUtil";
import type {
...
export function compareForRanking(
  a: DerivedStudentSnapshot & { previousRank: number | null },
  b: DerivedStudentSnapshot & { previousRank: number | null },
): number {
...
function avatarFor(studentId: string): { bg: string; text: string } {
...
function trendOf(rank: number, previousRank: number | null): TrendDirection {
  if (previousRank == null) return "new";
  if (previousRank === rank) return "same";
  return previousRank > rank ? "up" : "down";
}

function trendLabel(rank: number, previousRank: number | null): string | undefined {
  if (previousRank == null) return "New entry";
  if (previousRank === rank) return `Holding #${rank}`;
  if (previousRank > rank) return `Up from #${previousRank}`;
...
export function buildRanking(
  ...
  previousRankByStudentId: Map<string, number>,
): RankingEntry[] {
 ...
 */
export function updateRankHistory(
  existing: RankHistoryDoc | null,
  weekId: string,
  rank: number,
  score: number,
  maxWeeks: number,
): RankHistoryDoc["weeks"] {
  const prev = existing?.weeks ?? [];
  const filtered = prev.filter((w) => w.weekId !== weekId);
  filtered.push({ weekId, weekLabel: formatWeekShort(weekId), rank, score });
