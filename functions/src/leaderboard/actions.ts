...
import { getCurrentWeekId, weekIdToRange, isoDatesInRange } from "./weekUtil";
import type { ActionItem, InsightsDoc } from "./types";
 ...
 */
async function countOnTimeSubmissionsThisWeek(
  schoolId: string,
  studentId: string,
  classId: string,
  weekStart: number,
  ...
  const assignmentsSnap = await db
    ...
    .get();
  const dueIds = new Map<string, number>(); // assignmentId → due timestamp
  for (const a of assignmentsSnap.docs) {
    ...
    const due =
      (data.dueDate?.toMillis?.() ||
        ...
        0) as number;
    if (due >= weekStart && due <= weekEnd) dueIds.set(a.id, due);
  }
  if (dueIds.size === 0) return 0;
  ...
  for (const s of subsSnap.docs) {
    ...
    if (!aid || !dueIds.has(aid)) continue;
    const ts = (sd.submittedAt?.toMillis?.() || sd.timestamp?.toMillis?.() || 0) as number;
    if (ts && ts <= dueIds.get(aid)!) onTime++;
  }
  return onTime;
}

async function attendancePctThisWeek(
  schoolId: string,
  studentId: string,
  weekStart: number,
  weekEnd: number,
  ...
  const db = admin.firestore();
  const dates = isoDatesInRange(weekStart, weekEnd);
  const snap = await db
 ...
 */
async function bestTestScoreSince(
  schoolId: string,
  studentId: string,
  subject: string,
  sinceTs: number,
  ...
  for (const d of snap.docs) {
    const v = d.data();
    const ts = (v.timestamp?.toMillis?.() || 0) as number;
    ...
    const pct =
      typeof v.percentage === "number"
        ...
        : 0;
    if (best === null || pct > best) best = Math.round(pct);
  }
  return best;
}
...
async function refreshAction(
...
): Promise<{ next: ActionItem; changed: boolean }> {
  ...
  if (action.tracking === "auto_assignments") {
    const onTime = await countOnTimeSubmissionsThisWeek(
      ctx.schoolId,
      ctx.studentId,
      ctx.classId,
      ctx.weekStart,
    ...
    if (action.progress.current === 0) action.status = "pending";
    ...
    const pct = await attendancePctThisWeek(
      ctx.schoolId,
      ctx.studentId,
      ctx.weekStart,
      ctx.weekEnd,
    ...
    if (subject) {
      const best = await bestTestScoreSince(
        ctx.schoolId,
        ctx.studentId,
        subject,
        action.createdAt,
 ...
 */
export async function refreshActionsForStudent(
  studentId: string,
): Promise<{ updated: boolean; completedCount: number }> {
  const db = admin.firestore();
  ...
  if (!insightSnap.exists) return { updated: false, completedCount: 0 };
  const insight = insightSnap.data() as InsightsDoc;

  // Need schoolId + classId — fetch from students/{id}.
  const studentSnap = await db.doc(`students/${studentId}`).get();
  if (!studentSnap.exists) return { updated: false, completedCount: 0 };
  const sd = studentSnap.data() as { schoolId: string; classId: string };

  let anyChanged = false;
  const refreshed: ActionItem[] = [];
 ...
 */
export async function runActionTrackerCron(): Promise<void> {
