...
import { processClass } from "./cron";
import { generateInsightsForLeaderboard } from "./insights";
...
interface ManualTriggerInput {
  ...
  classId?: unknown;
  weekId?: unknown;
  generateInsights?: unknown; // optional: also fire insights generator
}
...
function callerSchoolId(token: { role?: string; schoolId?: string; uid?: string } | undefined): string | undefined {
  if (!token) return undefined;
  if (token.role === "owner") return token.schoolId || token.uid;
...
interface ManualTriggerResult {
  ...
  insights?: { generated: number; skipped: number; failed: number };
}
 ...
 */
export async function manualTriggerImpl(
  input: ManualTriggerInput,
  callerToken: { role?: string; schoolId?: string; uid?: string } | undefined,
  openaiApiKey: string,
  ...
  const result = await processClass(input.schoolId, input.classId, input.weekId);

  let insightsSummary: ManualTriggerResult["insights"];
  if (input.generateInsights === true) {
    // Re-read the freshly written leaderboard doc and run insights.
    const db = admin.firestore();
    const ref = db.doc(
      `leaderboards/${input.schoolId}_${input.classId}/weeks/${input.weekId}`,
    );
    const snap = await ref.get();
    if (snap.exists) {
      const lb = snap.data() as LeaderboardDoc;
      insightsSummary = await generateInsightsForLeaderboard(lb, openaiApiKey);
    }
  }
