...
export const FREE_ACCESS_CODES = new Set([
...
export function perStudentPrice(students: number): number {
...
export function calculatePrice(students: number, coupon?: string | null): PriceBreakdown {
  ...
  if (coupon && coupon.trim()) {
    ...
    if (FREE_ACCESS_CODES.has(code)) {
      ...
      const err: any = new Error(`Invalid coupon code: ${code}`);
