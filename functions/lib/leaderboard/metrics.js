 ...
 */
async function fetchPerStudentDual(collection, schoolId, studentId, studentEmail) {
    ...
    if (studentEmail) {
        queries.push(db.collection(collection)
            .where("schoolId", "==", schoolId)
 ...
 */
function readDueDate(data) {
    return (tsToMillis(data.dueDate) ||
 ...
 */
function noteSentiment(category, content) {
    ...
    if (POS.some((k) => haystack.includes(k)))
        return 1;
    if (NEG.some((k) => haystack.includes(k)))
        return -1;
    return 0;
}
/**
...
function readSubject(data) {
    ...
    if (/^hindi/.test(raw))
        return "hindi";
    if (/^social/.test(raw) || /^sst/.test(raw))
        return "social";
    return raw;
}
/**
 ...
 */
async function deriveStudentSnapshot(studentDoc, classAssignmentDocs, weekStart, weekEnd) {
    const data = studentDoc.data();
    const studentId = studentDoc.id;
    const schoolId = data.schoolId;
    const classId = data.classId;
    const name = data.name || "Unknown";
    const enrolledAt = tsToMillis(data.enrolledAt) || tsToMillis(data.createdAt) || Date.now();
    ...
    const studentEmail = (data.email || data.studentEmail || "").toString().trim().toLowerCase();
    const db = admin.firestore();
    ...
    for (const r of rows) {
        if (!subjectAccumulator[r.subject])
            subjectAccumulator[r.subject] = { sum: 0, n: 0 };
    ...
    const dateStrings = (0, weekUtil_1.isoDatesInRange)(weekStart, weekEnd);
    ...
    const presentDays = attendanceDocsThisWeek.filter((d) => d.data().status === "present").length;
    ...
    if (dueThisWeek.length > 0) {
        ...
        for (const s of subsDocs) {
            ...
            if (!ts || ts < weekStart - MS_PER_DAY * 14)
                continue; // ignore very old subs
    ...
    let behaviorScore = constants_1.DEFAULT_BEHAVIOR_SCORE;
    for (const n of noteDocs) {
        ...
        if (sentiment === 1)
            behaviorScore += constants_1.BEHAVIOR_DELTA_POSITIVE;
        else if (sentiment === -1)
            behaviorScore += constants_1.BEHAVIOR_DELTA_NEGATIVE;
    }
    ...
    const compositeScore = Math.round((breakdown.marks * constants_1.WEIGHTS.marks +
        breakdown.attendance * constants_1.WEIGHTS.attendance +
        breakdown.assignments * constants_1.WEIGHTS.assignments +
        breakdown.behavior * constants_1.WEIGHTS.behavior) *
        100) / 100;
 ...
 */
function aggregateSubjectAverages(snapshots) {
    const sums = {};
    for (const s of snapshots) {
        for (const [subj, score] of Object.entries(s.subjectScores)) {
            if (!sums[subj])
                sums[subj] = { total: 0, n: 0 };
