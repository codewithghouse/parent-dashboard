...
exports.generateInsightsForLeaderboard = void 0;
const admin = __importStar(require("firebase-admin"));
const axios_1 = __importDefault(require("axios"));
const constants_1 = require("./constants");
const sanitize_1 = require("./sanitize");
const SYSTEM_PROMPT = "You are a friendly mentor for school students in India. Be encouraging, " +
...
function actionRangeForRank(rank, total) {
    if (rank <= 3)
        return constants_1.ACTION_COUNT_BY_RANK_TIER.top3;
    if (rank <= Math.floor(total / 2))
        return constants_1.ACTION_COUNT_BY_RANK_TIER.midRange;
    if (rank <= Math.floor((3 * total) / 4))
        return constants_1.ACTION_COUNT_BY_RANK_TIER.lowerHalf;
    return constants_1.ACTION_COUNT_BY_RANK_TIER.struggling;
}
function classifySubjectStatus(score, classAvg) {
    if (score >= classAvg + 5)
        return "strong";
    if (score < classAvg - 2)
        return "weak";
...
function buildSubjectsArray(subjectMetrics) {
    if (!subjectMetrics)
        return [];
    const out = [];
    ...
    const order = { strong: 0, good: 1, weak: 2 };
    return out.sort((a, b) => order[a.status] - order[b.status]);
}
function buildTrajectoryArray(rankHistory, currentEntry) {
    const fromHistory = rankHistory?.weeks?.map((w) => ({
        weekId: w.weekId,
...
function buildUserPrompt(args) {
    ...
    const climbStr = trajLastFour.length >= 2
...
function validateAiPayload(p) {
    ...
    if (!Array.isArray(p.diagnosis) || p.diagnosis.length === 0)
        throw new Error("diagnosis missing");
    if (!Array.isArray(p.actions) || p.actions.length === 0)
        throw new Error("actions missing");
    if (!p.forecast || typeof p.forecast.projectedRank !== "number")
        throw new Error("forecast missing");
    for (const a of p.actions) {
        if (!a.id || !a.title || !a.tracking)
            throw new Error(`action missing fields: ${JSON.stringify(a)}`);
        // Repair the tracking field in-place if AI sent a pipe-separated string.
        const normalized = normalizeTracking(a.tracking);
        if (!normalized)
            throw new Error(`bad tracking: ${a.tracking}`);
...
function fallbackInsights(args) {
    ...
    const gaps = [
        { key: "marks", gap: breakdown.marks - classAvgBreakdown.marks },
        { key: "attendance", gap: breakdown.attendance - classAvgBreakdown.attendance },
        { key: "assignments", gap: breakdown.assignments - classAvgBreakdown.assignments },
        { key: "behavior", gap: breakdown.behavior - classAvgBreakdown.behavior },
    ...
    return {
        ...
        diagnosis: [
            {
                type: "good",
                text: (0, sanitize_1.sanitizeDiagnosisHtml)(`Tumhara strong area <strong>${gaps[gaps.length - 1].key}</strong> hai (${breakdown[gaps[gaps.length - 1].key]} score).`),
            },
            {
                type: "concern",
                text: (0, sanitize_1.sanitizeDiagnosisHtml)(`Sabse bada gap <strong>${weakest.key}</strong> mein hai — ${breakdown[weakest.key]} vs class avg ${classAvgBreakdown[weakest.key]}.`),
            },
        ],
...
async function generateForOneStudent(args) {
    ...
    try {
        ...
        const completionResp = await axios_1.default.post("https://api.openai.com/v1/chat/completions", {
            model: constants_1.OPENAI.model,
            temperature: constants_1.OPENAI.temperature,
            max_tokens: constants_1.OPENAI.maxTokens,
        ...
        return {
            ...
            diagnosis: parsed.diagnosis.map((d) => ({
                type: d.type,
                text: (0, sanitize_1.sanitizeDiagnosisHtml)(d.text),
            })),
            subjects,
            trajectory,
            topper: args.topper,
            actions: parsed.actions.map((a) => actionFromAi(a, now)),
            forecast: parsed.forecast,
            generatedAt: now,
            aiModel: constants_1.OPENAI.model,
        };
    }
    catch (err) {
 ...
 */
async function generateInsightsForLeaderboard(leaderboard, openaiApiKey) {
    ...
    const topper = {
        ...
        breakdown: topperEntry.breakdown ?? { marks: 0, attendance: 0, assignments: 0, behavior: 0 },
    };
    // Class average breakdown — average each metric across all rankings.
    const sumBreakdown = { marks: 0, attendance: 0, assignments: 0, behavior: 0 };
    let n = 0;
    ...
    let skipped = 0;
    let failed = 0;
    for (let i = 0; i < leaderboard.rankings.length; i += constants_1.OPENAI.batchSize) {
        const batch = leaderboard.rankings.slice(i, i + constants_1.OPENAI.batchSize);
        ...
        if (i + constants_1.OPENAI.batchSize < leaderboard.rankings.length) {
            await new Promise((r) => setTimeout(r, constants_1.OPENAI.delayBetweenBatchesMs));
        }
    }
    console.log(JSON.stringify({
...
exports.generateInsightsForLeaderboard = generateInsightsForLeaderboard;
