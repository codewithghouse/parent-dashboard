import { getMessagesIntelligencePrompt } from "../prompts/messages-prompt";

export async function generateMessageInsights(data: any): Promise<any> {
  const apiKey = import.meta.env.VITE_OPENAI_API_KEY;
  if (!apiKey) throw new Error("API Key missing");
  const prompt = getMessagesIntelligencePrompt(data);
