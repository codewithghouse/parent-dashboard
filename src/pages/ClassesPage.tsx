...
const ClassesPage = () => {
  ...
  return (
    <div className="animate-in fade-in duration-700 pb-24 text-left font-montserrat">
      ...
      {/* ── HEADER ── */}
      <div className="flex flex-col mb-6 md:mb-12 px-0 md:px-4 italic">
          <p className="text-[10px] font-black text-slate-900 uppercase tracking-[0.3em] opacity-80 mb-2">Institutional Service</p>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase leading-none">Academic Classes</h1>
          <p className="text-sm md:text-lg font-bold text-slate-400 mt-3 md:mt-4 lowercase">registered subject streams and faculty for {studentData?.name}</p>
      </div>
      ...
      {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8 lg:gap-10">
             {enrollments.map((en, idx) => (
                <div key={en.id} className="bg-white border border-slate-100 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 shadow-sm hover:shadow-2xl transition-all group relative overflow-hidden flex flex-col h-full">
                   {/* Background Decoration */}
                   <div className="absolute top-0 right-0 p-8 md:p-10 opacity-[0.03] scale-150 rotate-12 transition-transform duration-1000 group-hover:rotate-0">
                      <GraduationCap className="w-32 h-32 md:w-40 md:h-40 text-[#1e3a8a]" />
                   </div>

                   {/* Header Section */}
                   <div className="flex items-center gap-4 md:gap-6 mb-6 md:mb-10 relative z-10">
                      <div className={`w-14 h-14 md:w-20 md:h-20 rounded-[1.5rem] md:rounded-[2rem] ${cardColors[idx % cardColors.length]} flex items-center justify-center text-white text-2xl md:text-3xl font-black italic shadow-xl group-hover:rotate-6 transition-transform flex-shrink-0`}>
                         {en.initials}
                      </div>
                      <div className="min-w-0">
                         <h2 className="text-lg md:text-2xl font-black text-slate-900 tracking-tighter uppercase italic leading-none mb-2 truncate">{en.subject || "General"}</h2>
                   ...
                   {/* Details Section */}
                   <div className="space-y-4 md:space-y-6 flex-1 relative z-10 border-t border-slate-50 pt-5 md:pt-10">
                    ...
                    {/* Footer Actions */}
                    <div className="mt-6 md:mt-12 flex gap-3 md:gap-4 relative z-10">
      ...
      {/* ── FOOTER STATS ── */}
      <div className="mt-8 md:mt-16 bg-slate-900 rounded-[2rem] md:rounded-[3rem] p-6 md:p-10 text-white flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10">
