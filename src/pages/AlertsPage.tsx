...
import { AlertCircle, Clock, Trophy, Calendar, CheckCircle, User, Sparkles, Brain, ArrowRight, Loader2 } from "lucide-react";
...
const initialAlerts = [
  ...
  },
  {
    ...
    color: "amber",
    meta: "9:15 AM arrival",
  },
  {
    id: 3,
    title: "Great Improvement in Math!",
...
const AlertsPage = () => {
  ...
  const [aiStories, setAiStories] = useState<Record<number, any>>({});
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
           ...
           <div className="lg:col-span-8 space-y-6">
              {initialAlerts.map((alert) => (
                <div 
                  key={alert.id} 
                  className={`bg-white rounded-[2.5rem] border-2 border-slate-50 p-8 shadow-sm hover:shadow-xl transition-all relative overflow-hidden group ${isAllRead ? 'opacity-60' : ''}`}
                >
                       ...
                       <div>
                          ...
                          <div className="flex items-center gap-4">
                             <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <Clock className="w-3.5 h-3.5" /> {alert.meta}
                             </span>
                          </div>
                       </div>
                    </div>
                           ...
                           <div className="mt-8 flex flex-col md:flex-row items-center justify-between gap-6 p-6 bg-white/5 border border-white/10 rounded-3xl">
                              <div className="flex items-start gap-4">
                                 ...
                                 <div>
                                    ...
                                    <p className="text-sm font-bold text-white mt-1">{aiStories[alert.id].action_recommendation.text}</p>
                                 </div>
                              </div>
                              <button className="w-full md:w-auto px-8 py-3 bg-white text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-slate-100 transition-all">
                                 {aiStories[alert.id].action_recommendation.button_label} <ArrowRight className="w-4 h-4" />
                              </button>
                           </div>
                        </div>
                     </div>
           ...
           <div className="lg:col-span-4 space-y-6">
              <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white shadow-xl shadow-indigo-100 overflow-hidden relative">
                 ...
                 <div className="space-y-6 relative z-10">
                    ...
                    <p className="text-xs font-bold leading-relaxed opacity-80">
                       AI predicts that addressing "Assignments" tonight will improve Aditya's grade stability by <span className="text-indigo-200">12%</span>.
                    </p>
                 </div>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] border-2 border-slate-50 p-8 shadow-sm">
                 ...
                 <div className="space-y-4">
                    {[
                      "Check Science Project",
                      "Call Attendance Office",
                      "Sign Math test paper"
                    ].map((item, idx) => (
                       <label key={idx} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl cursor-pointer hover:bg-slate-100 transition-all border border-transparent hover:border-slate-200">
                          <input type="checkbox" className="w-5 h-5 rounded-lg border-2 border-slate-300 text-indigo-600 focus:ring-indigo-500" />
