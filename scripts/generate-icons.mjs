/**
 * Pure Node.js PNG icon generator — zero external deps.
...
function drawIcon(size) {
  ...
  // Rounded background #0B1F3A
  const r = Math.round(size * 0.18);
  const [BR, BG, BB] = [0x0B, 0x1F, 0x3A];
...
for (const s of sizes) {
  ...
  console.log(`✓ ${s}x${s}  →  ${name}`);
}

// Favicon 32x32
writeFileSync('./public/favicon-32x32.png', makePNG(32, 32, drawIcon(32)));
console.log('✓ 32x32  →  public/favicon-32x32.png');
